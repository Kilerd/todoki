            ...
            <div className="flex items-center justify-between p-2 rounded-sm hover:bg-gray-100 group">
                <div className="flex items-center gap-2">
                    ...
                    {props.task_type === "Todo" && 
                        <Checkbox disabled={props.archived} checked={checked} onClick={() => handleClick(!checked)} />}
                    {props.task_type === "Stateful" &&
                        <Badge variant="secondary">{props.current_state}</Badge>}
                    
                    <Link to={`/tasks/${props.id}`}
                        className={`leading-7 ${props.done || props.archived ? 'line-through text-gray-500' : ''}`}>
                ...
                <div className="hidden group-hover:flex items-center gap-2">
                    {(!props.archived && prevState !== undefined) &&
                        <Button variant="secondary" size="sm" onClick={() => handleUpdateState(prevState)}>
                            Back to {prevState}
                        </Button>
                    }
                    ...
                    {props.archived ?
                        <Button variant="ghost" size="icon" onClick={() => unarchiveTask(props.id)}>
                    ...
                    <Button variant="ghost" size="icon" onClick={() => deleteTask(props.id)}>
                        <Trash2 className="h-4 w-4" />
                    </Button>
                </div>
            </div>
            {day_events.map(event => (
                <div key={event.datetime} className="flex items-center gap-2 ml-14 text-gray-500 text-sm py-0.5">
