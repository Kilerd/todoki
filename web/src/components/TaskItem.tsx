        ...
        <div className="flex items-center justify-between py-1 px-2 rounded-lg hover:bg-gray-50 group">
            <div className="flex items-center gap-2 py-1">
                ...
                {isLoading ? <Loader2 className="h-5 w-5 animate-spin" /> :
                    <>
                        {props.task_type === "Todo" &&
                            <Checkbox className="h-5 w-5 bg-gray-100 " checked={checked} onCheckedChange={handleClick} />}
                ...
                <Link to={`/tasks/${props.id}`}
                    className={cn("text-gray-900 no-underline leading-7", props.done || props.archived ? 'line-through text-gray-500' : '')}>
                    {props.priority > 0 &&
                        <span className="text-red-900 font-bold pr-2">{"!".repeat(props.priority)}</span>}
                    {props.content}
                </Link>
                ...
                {standDuration > 0 &&
                    <Badge variant="destructive">
                        任务持续超过 {standDuration} 天
                    </Badge>}
            </div>

            <div className="hidden group-hover:flex items-center">
                {prevState !== undefined &&
                    <Button variant="ghost" size="sm" onClick={() => handleUpdateState(prevState)}>
                        Back to {prevState}
                    </Button>
                }
                {nextState !== undefined &&
                ...
                <Button variant="ghost" size="icon" onClick={() => archiveTask(props.id)}>
                    <Archive className="h-4 w-4" />
                </Button>
                <Button variant="ghost" size="icon" className="text-pink-600" onClick={() => deleteTask(props.id)}>
